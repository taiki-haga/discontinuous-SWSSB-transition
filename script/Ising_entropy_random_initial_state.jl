# ────────────────────────────────────────────────────────────────────────────────
# TFIM + (x or z)-dephasing : Evolution of von Neumann Entropy
...
function renyi_entropy(ρ::Matrix{ComplexF64})
    return -log(real(tr(ρ^2)))
end

function simulate_entropy(seed::Integer;
    ...
    # store entropy
    S_1 = zeros(Float64, length(tkeep))
    S_2 = zeros(Float64, length(tkeep))
    ...
    idx = 1
    S_1[idx] = von_neumann_entropy(ρ)
    S_2[idx] = renyi_entropy(ρ)
...
# total time
tmax = 2.0
# compute Entropy every 'stride' steps (>=1)
stride = 1
# =================================
...
for seed = 2:2
    println("  seed = ", seed)
    t, S_1, S_2, ρ_f = simulate_entropy(seed;
        L=L, J=J, h=h, γ=γ, axis=axis, dt=dt, tmax=tmax, stride=stride)

    save("data_paper//Ising_S_evolution//S_J=$(J)_h=$(h)_L=$(L)_seed=$(seed).jld", "t", t, "S", S_1)
    save("data_paper//Ising_S_evolution//S2_J=$(J)_h=$(h)_L=$(L)_seed=$(seed).jld", "t", t, "S", S_2)
end
