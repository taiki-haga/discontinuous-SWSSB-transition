...
# ----- Initial states -----
function build_rainbow_state(L::Int)
    if L % 2 != 0
        error("System size L must be even to construct a Rainbow state.")
    end
    ...
    for i in 0:(2^half_L-1)
        ...
        for k in 0:(half_L-1)
            ...
            if bit == 1
                full_state_index |= (1 << left_pos)
                full_state_index |= (1 << right_pos)
            end
        end

        psi[full_state_index+1] = 1.0
    end
...
ρ0 = build_rainbow_state(L)

t, R2, ρ_f = simulate_Rbar2(ρ0; L=L, J=J, hx=hx, hz=hz, γ=γ, axis=axis, dt=dt, tmax=tmax, stride=stride)

save("data//Ising//rainbow_initial_state//R2_J=1.0_h=0.0_L=$(L).jld", "t", t, "R2", R2)
