# ────────────────────────────────────────────────────────────────────────────────
#  Time evolution of Renyi-2 correlator 
...
# ───────────────────────────────────────────────
# 7. Rényi-2 correlator and Binder cumulant
# ───────────────────────────────────────────────
function renyi2_bar(ρ::Matrix{ComplexF64})
    purity = real(dot(ρ, ρ))   # Tr[ρ²]
    num = 0.0
    for i in 1:L, j in 1:L
        num += real(tr(ρ * BdB[i, j] * ρ * BdB[j, i]))
    end
    return num / (L^2 * purity)
end

function binder_numerator(ρ::Matrix{ComplexF64})
    purity = real(dot(ρ, ρ))
    acc = 0.0
    @inbounds for i in 1:L, j in 1:L, k in 1:L, l in 1:L
        left = BdB[i, j] * BdB[k, l]
        right = BdB[l, k] * BdB[j, i]
        acc += real(tr(ρ * left * ρ * right))
    end
    return acc / (L^4 * purity)
end

function binder_cumulant(ρ::Matrix{ComplexF64})
    r2 = renyi2_bar(ρ)
    b4 = binder_numerator(ρ)
    return 1 - b4 / (3 * r2 * r2)
end
...
# ───────────────────────────────────────────────
function renyi_evolution(steps::Int, seed::Int)
    Random.seed!(seed)
    ...
    renyi = zeros(Int(steps / samp_int) + 1)
    #binder = zeros(Int(steps / samp_int))

    @showprogress for idx in 0:steps
        if idx % samp_int == 0
            renyi[Int(idx / samp_int)+1] = renyi2_bar(ρ)
            #binder[Int(idx / samp_int)+1] = binder_cumulant(ρ)
        end
        rk4_step!(ρ, dt, k_rhs, k_mid)
    end

    return times, renyi #, binder
end
...
for seed in 9:10
    println("  seed = ", seed)
    t, R2 = renyi_evolution(steps, seed)
    save("data//HCB//random_initial_state//R2_J=$(J)_L=$(L)_N=$(Npart)_seed=$(seed).jld", "t", t, "R2", R2)
end
