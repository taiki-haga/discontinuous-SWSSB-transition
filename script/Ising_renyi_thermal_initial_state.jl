...
# ----- Initial states -----
function build_thermal_state(L, seed)
    Random.seed!(seed)
    D = 2^L
    #hx = 0.1randn(L) .+ 1.0 # thermal
    #hz = 0.1randn(L) .+ 1.0
    hx = 10.0randn(L) # MBL
    hz = 10.0randn(L)

    H, Z, X = build_TFIM(L, 1.0, hx, hz)

    # Z_2 symmetry operator U = Π X_n
    U = build_U(X)

    # Diagonalization
    lambda, v = eigen(Matrix(H))

    # Eigenstate at the center of the spectrum
    idx = div(D, 2) + rand(-div(D, 8):div(D, 8))
    ψ = v[:, idx]
    ψ = ψ + U * ψ
    ψ /= norm(ψ)
...
for seed in 91:100
    println(" seed = ", seed)
    ρ0 = build_thermal_state(L, seed)
    t, R2, ρ_f = simulate_Rbar2(ρ0; L=L, J=J, hx=hx, hz=hz, γ=γ, axis=axis, dt=dt, tmax=tmax, stride=stride)
    save("data_paper//Ising_R_evolution//MBL_initial_state//R2_J=1.0_h=0.0_L=$(L)_seed=$(seed).jld", "t", t, "R2", R2)
end
